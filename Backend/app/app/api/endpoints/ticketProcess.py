...
@router.post(
    "/setting-intial-process/",
    description="user can create the process state of the ticket",
...
async def createTicketProcess(
    ...
    ticket_assigned = getTicketAssigned(db=db, ticket_id=ticket_process.ticket_id)
    ...
    if (
        ...
        or ticket_assigned.service_engineer_id != current_user.id
    ):
...
@router.put(
    ...
    description="the user can change the process details",
    response_model=Message,
)
async def updateTicketProcess(
    ...
    if not db_ticket_process:
        raise HTTPException(
            status_code=400, detail="This tickect has no process history"
        )
...
async def addSpareParts(
    ...
    if not db_ticket_process:
        raise HTTPException(
            status_code=403, detail=f"This user doesn't handle ticket no: {ticket_id}."
        )

    if not db_ticket_process.spare_parts_required:
        raise HTTPException(
            status_code=403, detail="This ticket is not eligible for spare parts."
        )
...
async def listOfRequestedSpareParts(
    ...
    if current_user.user_type == 1:
        ticket_requests = (
            db.query(SpareParts).filter(SpareParts.status == "pending").all()
        )
    else:
        service_engineers = getServiceEngineersUnderHead(
            db=db, current_user=current_user
        )
        if not service_engineers:
            raise HTTPException(
                status_code=404,
                detail="No service engineers found under this service head",
            )

        service_engineer_ids = [se.id for se in service_engineers]
        ticket_requests = getSparePartRequests(
            db=db, service_engineer_ids=service_engineer_ids
        )
...
@router.get(
    "/list-spare-parts/{ticket_id}",
    response_model=List[SparePartResponse],
    description="its shows the sapre parts",
)
...
@router.put(
    "/spare-parts/{ticket_id}/bulk-action",
    response_model=SparePartsBulkActionResponse,
    description="Admin or service head can approve or reject the spare part rejection",
)
...
@router.put(
    "/tickets/status-changes/",
    response_model=Message,
    description="it change the ticket status as complete or cancelled",
)
async def changeStatussTicket(
    ...
    if current_user.type_id == 1:
        completed_ticket = statusTicketInDb(
            db=db, ticket_id=ticket.ticket_id, status=ticket.status
        )
        ...
        if db_assigned_ticket.service_engineer.report_to == current_user.id:
            ...
            raise HTTPException(
                status_code=403, detail="this tickect has another service head"
            )
