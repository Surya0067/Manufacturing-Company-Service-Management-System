...
async def CreateWorkReport(
    ...
    existing_report = getWorkReportByDate(
        db=db, user_id=current_user.id, report_date=report.report_date
    )
    if existing_report:
        raise HTTPException(
            status_code=400, detail="A report for this date has already been submitted."
        )
    if db_report:
        if report.report_date < db_report.report_date:
            raise HTTPException(
                status_code=400,
                detail="A report for this date has already been submitted.",
            )
...
async def myWorkReports(
    db: Session = Depends(get_db), current_user: User = Depends(getCurrentUser)
):
    work_reports = viewWorkReportForSpecificUser(
        db=db, username=current_user.username, current_user=current_user
    )
    if work_reports:
        return work_reports
...
async def viewWorkReports(
    username: str,
    db: Session = Depends(get_db),
    current_user: User = Depends(serviceHeadLogin),
):
    work_reports = viewWorkReportForSpecificUser(
        db=db, username=username, current_user=current_user
    )
    if work_reports:
        return work_reports
    raise HTTPException(404, "Work reports not found")
