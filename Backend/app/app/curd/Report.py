...
def getWorkReportByDate(db : Session,user_id : int , report_date : date):
    return db.query(WorkReport).filter(WorkReport.user_id == user_id,WorkReport.report_date == report_date).first()
...
def viewWorkReportForSpecificUser(db : Session,username : int,current_user : User):
    ...
    return db.query(WorkReport).filter(WorkReport.user_id == user.id).order_by(WorkReport.report_date.desc()).all()

def viewWorkReportForSpecificUserDateRange(db: Session, username: str, current_user: User, start_date: date, ending_date: date):
    user = userVaildation(db=db,username=username,current_user=current_user)
    ...
    ).order_by(WorkReport.report_date.desc()).all()
