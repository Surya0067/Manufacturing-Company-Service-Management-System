...
def generateTicketReportpdf(ticket_id: int, db: Session, file_path: str):
    ...
    report_data.append(
        f"Status: {'Rejected' if ticket.ticket_rejected else 'In Progress'}"
    )
    ...
    if ticket.ticket_rejected:
        ...
        if latest_assignment:
            report_data.append(
                f"Assigned To: {latest_assignment.service_engineer.username}"
            )
        ...
        if ticket.ticket_process:
            ...
            report_data.append(
                f"Expected Completion Date: {process.excepted_complete_date}"
            )
            report_data.append(
                f"Actual Completion Date: {process.actual_complete_date or 'Not Completed'}"
            )
            report_data.append(
                f"Spare Parts Required: {'Yes' if process.spare_parts_required else 'No'}"
            )

            total_spare_cost = 0
            if process.spare_parts_required:
                spare_parts = (
                    db.query(SpareParts).filter(SpareParts.ticket_id == ticket_id).all()
                )
                total_spare_cost = 0
                ...
                for part in spare_parts:
                    ...
                    report_data.append(
                        f"  - {part.part_name} (Quantity: {part.quantity}, Unit Price: {part.unit_price}, Cost: {cost})"
                    )
...
def getAssignedTickets(
    service_head_id: int, db: Session, current_month: int, current_year: int
):
    service_engineer_alias = aliased(User)

    return (
        db.query(TicketProcess)
        .select_from(TicketProcess)
        .join(TicketAssign, TicketProcess.ticket_id == TicketAssign.ticket_id)
        .join(
            service_engineer_alias,
            TicketAssign.service_engineer_id == service_engineer_alias.id,
        )
        .filter(
...
def getTeamMemberCount(service_head_id: int, db: Session):
    return (
        db.query(User)
        .filter(User.report_to == service_head_id)
        .distinct(User.id)
        .count()
    )
...
def getServiceEngineerPerformance(db: Session, username: str):
    ...
    assigned_tickets = (
        db.query(TicketAssign)
        .filter(TicketAssign.service_engineer_id == engineer.id)
        .count()
    )
    completed_tickets = (
        db.query(TicketAssign)
        .filter(
            TicketAssign.service_engineer_id == engineer.id,
